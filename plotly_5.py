...
fig_subplot = make_subplots(
    rows=1, cols=3,
    subplot_titles=("아델리","젠투","턱끈")
)
# 함수도움말 보는법: 함수이름뒤에 ?붙여서 실행
#make_subplots? 
...
fig_subplot.update_layout(
    title=dict(text="펭귄종별 부리길이 vs. 깊이", x=0.5)
)
...
fig = make_subplots(
 ...
 #vertical_spacing 위 아래 여백
 #horizontal_spacing 플랫끼리 여백
 #specs로 전체자리를 차지하면 나머지 자리는 none처리를 해줘야 한다
